...
class DbomNode {
     ...
     * @param {string=} signServiceURI - The URI of the signing service. If not provided,
    ...
    async getAsset(repoID, channelID, assetID) {
        ...
        try {
            ...
            let status = this._getErrorStatus(e)
            if (status === 404)
    ...
    async _signAsset(payload) {
        if (this.signServiceURL === null)
            throw new SigningServiceError('URI of signing service unspecified')
        let {
            attachedChildren,
            parentAsset,
        ...
        try {
            let signatureResponse = await axios.post(this.signServiceURL, filteredPayload)
            payload['manufactureSignature'] = signatureResponse.data['signature']
            payload['assetMetadata']['manufactureFingerprint'] = signatureResponse.data['fingerprint']
            return payload
        } catch (e) {
    ...
    async _pushToGateway(repoID, channelID, assetID, payload, isUpdate = false) {
        ...
        if (!validPayload)
            throw new AssetInvalid('Schema Invalid', this.signedAssetValidator.errors)

        if (this.signServiceURL !== null)
            payload = await this._signAsset(payload)

        const requestURI = this._constructAssetURL(repoID, channelID, assetID)
        try {
            if (isUpdate)
                return await axios.put(requestURI, payload)
            else
                return await axios.post(requestURI, payload)
        } catch (e) {
            let status = this._getErrorStatus(e)
            if (status === 409)
    ...
    async attachSubasset(parentRepoID, parentChannelID, parentAssetID,
        ...
        try {
            ...
            if (status === 403)
                ...
                throw new ParentAssetNotFound()
            else if (status === 502 && e.response.data['error'].includes('not found'))
                throw new ChildAssetNotFound()
            else
                throw new GatewayError(status, e.response.data)
        }
    }
